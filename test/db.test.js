...
describe("Database Functions", function () {
  ...
  describe("Single Entry", function () {
    ...
    it("Remove Entry with incorrect data", function (done) {
      db.change(table, { test_name: name }).then((data) => {
        assert.ok(data.rows == 1);
        db.remove(table, [[["my_name", "=", name]]]).then((data) => {
          assert.ok(
            data.message == "Unknown column 'my_name' in 'where clause'"
          );
          done();
        });
      });
  ...
  describe("Bulk Entry", function () {
    ...
    it("List with incorrect data", function (done) {
      db.list(table, [[["unknown_column", "=", "Mr."]]], 1).then((data) => {
        assert.ok(
          data.message == "Unknown column 'unknown_column' in 'where clause'"
        );
        done();
      });
    });
    it("List with invalid object", function (done) {
      db.list(table, { unknown_column: "Mr." }, 1).then((data) => {
        assert.ok(data.message == "Invalid filter object");
        done();
      });
    });
    it("Get specific entries with incorrect entries", function (done) {
      db.get(table, [[["id", "in", ["8", "9", "10", "11", "12"]]]]).then(
        (data) => {
